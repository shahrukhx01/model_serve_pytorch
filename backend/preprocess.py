...
class Preprocess:
    ...
    def perform_preprocessing(self, text):       
        clean_text = text.lower() ## normalizing text to lower case
        clean_text = self.remove_usernames(clean_text) ## removing usernames
        clean_text = self.remove_punctuations(clean_text) ## removing punctuations
        clean_text = self.remove_stopwords(clean_text) ## removing stopwords
